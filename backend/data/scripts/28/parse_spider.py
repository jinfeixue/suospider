...
DB_USER = "root"
DB_PASSWORD = "123456"
SPIDER_NAME = "spider_28_9b4d08d4"
...
def parse():
    ...
    for record_id, url in urls:
        ...
        try:
            vals = tree.xpath("//h1/text()")
            data["title"] = vals[0].strip() if vals else ""
        except:
            data["title"] = ""
        try:
            vals = tree.xpath("//meta[@name='author']/text()")
            data["zuozhe"] = vals[0].strip() if vals else ""
        except:
            data["zuozhe"] = ""
        try:
            vals = tree.xpath("//div[contains(@class, 'field--name-body')]//text()")
            data["content"] = vals[0].strip() if vals else ""
        except:
            data["content"] = ""
        try:
            vals = tree.xpath("//div[contains(@class, 'usa-identifier__identity-disclaimer')]/a/text()")
            data["kanming"] = vals[0].strip() if vals else ""
        except:
            data["kanming"] = ""
        try:
            vals = tree.xpath("//h2[contains(text(), 'Keywords')]/following-sibling::ul[1]//a/text()")
