...
DB_USER = "root"
DB_PASSWORD = "123456"
SPIDER_NAME = "spider_41_b93df998"
TASK_NAME = "usa_nasa"
...
def parse():
    ...
    for record_id, url in urls:
        try:
            ...
            try:
                vals = tree.xpath("//article[@itemtype='http://schema.org/Article']//h1/text()")
                texts = []
                for v in vals:
                    if isinstance(v, str):
                        t = v.strip()
                ...
                data["title"] = ""
            try:
                vals = tree.xpath("//article[@itemtype='http://schema.org/Article']//div[contains(@class,'single-blog-content')]/p//text()")
                texts = []
                for v in vals:
                    if isinstance(v, str):
                        t = v.strip()
                ...
                data["content"] = ""
            try:
                vals = tree.xpath("//article[@itemtype='http://schema.org/Article']//time[@itemprop='datePublished']/text()")
                texts = []
                for v in vals:
                    if isinstance(v, str):
                        t = v.strip()
                ...
                data["chubanriqi"] = ""
            try:
                vals = tree.xpath("//article[@itemtype='http://schema.org/Article']//p[@itemprop='author']/text()")
                texts = []
                for v in vals:
                    if isinstance(v, str):
                        t = v.strip()
                ...
                data["zuozhe"] = ""
            try:
                vals = tree.xpath("//nav[@id='categories-sidebar']/a/text()")
                texts = []
                for v in vals:
                    if isinstance(v, str):
                        t = v.strip()
                ...
                data["keyword"] = ""
            try:
                vals = tree.xpath("//div[@id='blog-name-header']//h2/text()")
                texts = []
                for v in vals:
                    if isinstance(v, str):
                        t = v.strip()
