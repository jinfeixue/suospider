...
CONFIGURED_FIELDS = {"title": "//article[@role='region']//h1/text()", "content": "//article[@role='region']//div[contains(@class,'single-blog-content')]//text()[not(ancestor::h1)]", "chubanriqi": "//article[@role='region']//time[@datetime][1]/text()", "zuozhe": "//p[contains(@class,'author-name')][1]/text()", "keyword": "//div[contains(@class,'categories-links')]//a[contains(@class,'category-link')]/text()", "kanming": "//div[@id='blog-name-header']//h2/a/text()"}
...
def trans_date(value):
    ...
    # 处理重复日期（如 "June 9, 2026 3:57PM June 9, 2026 3:57PM"）
    parts = s.split()
    if len(parts) > 6:
        mid = len(parts) // 2
...
def call_llm(prompt):
    ...
    try:
        ...
        import os
        _app_db_host = os.environ.get("SPIDER_DB_HOST", DB_HOST)
        _app_db_port = int(os.environ.get("SPIDER_DB_PORT", DB_PORT))
        _app_db_name = os.environ.get("SPIDER_DB_NAME", "spider")
        _app_db_user = os.environ.get("SPIDER_DB_USER", DB_USER)
        _app_db_password = os.environ.get("SPIDER_DB_PASSWORD", DB_PASSWORD)
