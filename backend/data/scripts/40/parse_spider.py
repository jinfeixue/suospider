...
def parse():
    ...
    for record_id, url in urls:
        try:
            ...
            try:
                vals = tree.xpath("//h1[contains(@class, 'margin-bottom-4')]/text()")
                texts = []
                for v in vals:
                    if isinstance(v, str):
                        t = v.strip()
            ...
            try:
                ...
                data["content"] = ""
            try:
                vals = tree.xpath("(//div[contains(@class, 'blog-sidebar')]//time/text())[1]")
                texts = []
                for v in vals:
                    if isinstance(v, str):
                        t = v.strip()
                ...
                data["chubanriqi"] = ""
            try:
                vals = tree.xpath("//p[@itemprop='author']/text()")
                texts = []
                for v in vals:
                    if isinstance(v, str):
                        t = v.strip()
