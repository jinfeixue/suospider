...
def parse():
    ...
    for record_id, url in urls:
        try:
            ...
            try:
                vals = tree.xpath("//h2[@id='ivs_title']/text()[1]")
                texts = []
                for v in vals:
                    if isinstance(v, str):
                        t = v.strip()
                ...
                data["title"] = ""
            try:
                vals = tree.xpath("//div[@id='ivs_content']//text()")
                texts = []
                for v in vals:
                    if isinstance(v, str):
                        t = v.strip()
                ...
                data["content"] = ""
            try:
                vals = tree.xpath("//small[@id='ivs_date']/text()")
                texts = []
                for v in vals:
                    if isinstance(v, str):
                        t = v.strip()
                ...
                data["chubanriqi"] = ""
            try:
                vals = tree.xpath("//div[@id='ivs_content']/p[contains(text(),'记者')]/text()")
                texts = []
                for v in vals:
                    if isinstance(v, str):
                        t = v.strip()
                        ...
                        if t and len(t) > 3:
                            texts.append(t)
                data["zuozhe"] = " ".join(texts) if texts else ""
            except:
                data["zuozhe"] = ""
            try:
                ...
                for v in vals:
                    if isinstance(v, str):
                        ...
                        if t and len(t) > 3:
                            texts.append(t)
                data["keyword"] = " ".join(texts) if texts else ""
            except:
                data["keyword"] = ""
            try:
                vals = tree.xpath("//small[@id='ivs_date']/span/text()")
                texts = []
                for v in vals:
                    if isinstance(v, str):
                        t = v.strip()
