    ...
    for record_id, url in urls:
        try:
            ...
            try:
                ...
                data["title"] = ""
            try:
                vals = tree.xpath("//div[@id='single-blog-1004374' and contains(@class, 'single-blog-content')]/div[not(@class)]//text()")
                texts = []
                for v in vals:
                    if isinstance(v, str):
                        t = v.strip()
            ...
            try:
                ...
                data["chubanriqi"] = ""
            try:
                vals = tree.xpath("//div[contains(@class, 'blog-sidebar')]//p[contains(@class, 'author-name')]/text()")
                texts = []
                for v in vals:
                    if isinstance(v, str):
                        t = v.strip()
