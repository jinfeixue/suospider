...
def parse():
    ...
    for record_id, url in urls:
        ...
        try:
            vals = tree.xpath("//h1[contains(@class,'page-title')]/text()")
            data["title"] = vals[0].strip() if vals else ""
        except:
            data["title"] = ""
        try:
            vals = tree.xpath("//div[contains(@class,'field--name-body')]")
            data["content"] = vals[0].strip() if vals else ""
        except:
            data["content"] = ""
        try:
            vals = tree.xpath("//span[@class='news-release-date-value']/text()")
            data["chubanriqi"] = vals[0].strip() if vals else ""
        except:
            data["chubanriqi"] = ""
        try:
            vals = tree.xpath("//ul[contains(@class,'usa-collection__meta') and .//h2[contains(text(),'Keywords')]]/li/a/text()")
            data["keyword"] = vals[0].strip() if vals else ""
        except:
            data["keyword"] = ""
        try:
            vals = tree.xpath("//meta[@name='description']/@content")
            data["abstract"] = vals[0].strip() if vals else ""
        except:
            data["abstract"] = ""
        try:
            vals = tree.xpath("//meta[@property='og:site_name']/@content")
