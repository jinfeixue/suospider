...
DB_USER = "root"
DB_PASSWORD = "123456"
SPIDER_NAME = "spider_24_9b4d08d4"
...
def parse():
    ...
    for record_id, url in urls:
        ...
        try:
            ...
            data["title"] = ""
        try:
            vals = tree.xpath("(//h1[contains(@class, 'page-title')]/following::div[contains(@class, 'field--name-body')])[1]//text()")
            data["content"] = vals[0].strip() if vals else ""
        except:
            data["content"] = ""
        try:
            vals = tree.xpath("//h2[contains(text(), 'Keywords')]/following-sibling::li/a/text()")
