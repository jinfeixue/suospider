...
MAX_PAGES = 2
INTERVAL = 2
DETAIL_LINK_XPATH = "//ul[contains(@class,'tadaty-list')]/li/a/@href"
PAGINATION_TYPE = "url_pattern"
NEXT_XPATH = '//div[@class="whj_jqueryPaginationCss-1"]'
PAGE_URL_TEMPLATE = "index_{{page}}.html"
PAGE_COUNT = 0
LLM_CONFIG_ID = 1

# 任务配置的字段（专业采集配置的 fields 优先使用）
CONFIGURED_FIELDS = {"title": "//h2[@id='ivs_title']/text()", "zuozhe": "//div[@id='ivs_content']/p[1]/text()", "content": "//div[@id='ivs_content']//text()", "kanming": "//*[@id='ivs_date']/span/text()", "keyword": "//meta[@name='keywords']/@content", "chubanriqi": "//*[@id='ivs_date']/text()[1]"}
...
DB_USER = "root"
DB_PASSWORD = ""
SPIDER_NAME = "spider_33_a32f16a3"
    ...
    for page_num in range(1, MAX_PAGES + 1):
        ...
        try:
            ...
            if PAGINATION_TYPE == "next_button" and NEXT_XPATH:
                ...
                current_url = PAGE_URL_TEMPLATE.replace("{page_num}", str(page_num + 1)).replace("{page}", str(page_num + 1))
            else:
                break
        ...
        for page_num in range(1, max_page + 1):
            ...
            try:
                ...
                if page_num < max_page:
                    ...
                    if not clicked:
                        try:
                            page.locator(f"a:text-is('{page_num + 1}')").first.click()
                            clicked = True
                        except:
                            pass
                    # 方式3：JavaScript直接调用
                    if not clicked:
                        try:
                            page.evaluate(f"if(typeof goPage==='function') goPage({page_num + 1}); else if(typeof page==='function') page({page_num + 1});")
                            clicked = True
                        except:
                            pass

                    if not clicked:
                        log(f"无法点击第{page_num + 1}页，停止翻页")
                        break
